...
## Written By https://github.com/OTU1NTA1
...
if [ -n "$ipv4_interface" ]; then
    ipv4_info=$(ip addr show dev $ipv4_interface | grep 'inet ')
    if [ -n "$ipv4_info" ]; then
...
real_grub=$(__get_grub "/boot")
[ -z "$real_grub" ] && __fatal "Error! Not Found grub."
grub_dir=$(echo "$real_grub" | cut -d':' -f1)
grub_file=$(echo "$real_grub" | cut -d':' -f2)
grub_version=$(echo "$real_grub" | cut -d':' -f3)
grub_temp_file=$(mktemp)

__log 'Checking grub.cfg ...'
[[ ! -f "${grub_dir}/${grub_file}" ]] && __fatal "Error! Not Found ${grub_dir}/${grub_file} ..."

__log 'Backing up and restoring GRUB configuration files ...'
[[ ! -f "${grub_dir}/${grub_file}.old" && -f "${grub_dir}/${grub_file}.bak" ]] && mv -f "${grub_dir}/${grub_file}.bak" "${grub_dir}/${grub_file}.old"
mv -f "${grub_dir}/${grub_file}" "${grub_dir}/${grub_file}.bak"
cat "${grub_dir}/${grub_file}.old" >"${grub_dir}/${grub_file}" 2>/dev/null || cat "${grub_dir}/${grub_file}.bak" >"${grub_dir}/${grub_file}"

__log 'looking grub.cfg  menuentry ...'
# Get the first menuentry block
sed -n '/^menuentry /{:a;N;/\}$/!ba;p;q;}' "$grub_dir/$grub_file" | sed '/^\s*$/d' > "$grub_temp_file"
...
insert_row_num=$(awk '/^menuentry /{print NR; exit}' "$grub_dir/$grub_file");
...
sed -i ''${insert_row_num}'r '${grub_temp_file}'' $grub_dir/$grub_file;
